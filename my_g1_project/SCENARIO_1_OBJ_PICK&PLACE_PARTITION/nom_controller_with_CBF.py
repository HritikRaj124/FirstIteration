...
# Global Variables and Task Configurations
...
APPROACH, GRASP, TRANSIT, RETREAT = 0.10, 0.03, 0.05, 0.15 # Waypoints determining movements (used to update the z-axis of the object_pos)

PHASE_DURATION = 2.5  # time for every phase of the movement for the waypoints
CONTROL_DT = 0.01     # time-step
ARM_SAFETY_RADIUS = 0.03  # Safety net layer radius
GRASP_RADIUS = 0.08
REALTIME = True # for the viewer
...
# writes a dictionary of joint actuator transmission IDs to their corresponding actuator indices.
...
def minjerk(s):
    return 10 * s**3 - 15 * s**4 + 6 * s**5  # 5th degree polynomial 
...
def update_grasp(data, palm_pos, obj_qadr, obj_dadr, active, grasped, object_z_axis):
    if not active:
        return False
    ...
    if grasped:
        held = palm_pos.copy()
        held[2] = max(held[2], object_z_axis)
...
def run_scenario(use_filter: bool, show_viewer: bool = True) -> list[dict]:
    """This one will run full pick-place rollout, either with the CBF safety filter
    ...
    partition_geom = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, PARTITION_GEOM)
    ...
    arm_actuators = np.array([actuators_map[mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, n)] for n in LEFT_ARM_JOINT_NAMES])
    ...
    hold = [(aid, float(data.qpos[model.jnt_qposadr[jid]])) for jid, aid in actuators_map.items() if aid not in arm_set]

    p_center = data.geom_xpos[partition_geom].copy()
    p_half = model.geom_size[partition_geom].copy()

    table_top = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, "table_top")
    if table_top < 0:
        raise ValueError("Unknown geom: table_top")
    t_center = data.geom_xpos[table_top].copy()
    t_half = model.geom_size[table_top].copy()

    object_pos = data.xpos[mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, OBJECT_BODY)].copy()
    ...
    print(f"object {np.round(object_pos, 3)}   place {np.round(place_pos, 3)}")

    # building the CBF barrier functions once reusing the live obstacle geometry above
    ...
    waypoints = [
        ("approach",      object_pos + [0, 0, APPROACH]),
        ("descend",       object_pos + [0, 0, GRASP]),
        ("transit_start", object_pos + [0, 0, TRANSIT]),
    ...
    grasped = False
    distance_during_descend = []

    viewer = mujoco.viewer.launch_passive(model, data) if SHOW_VIEWER else None
    try:
        for phase, q_end in plan:
            for i in range(n_steps):
                q_ref = q_start + minjerk((i+1) / n_steps) * (q_end - q_start) # Trajectory Interpolator

                q_current = data.qpos[qpos_ids].copy()
                arm_q = jnp.array(q_current)

                # compute h(q) for both obstacles cheap
                h_val_partition = float(h_forearm(arm_q))
                ...
                if USE_SAFETY_FILTER:
                    ...
                    qdot_safe = solve_safety_qp( qdot_des,[h_val_partition, h_val_table],[grad_h, grad_h_table], ALPHA, QDOT_MAX,)
                ...
                for _ in range(n_sub):
                    ...
                    grasped = update_grasp(data, data.site_xpos[site_id].copy(), obj_qadr, obj_dadr, phase in GRASP_PHASES, grasped, object_z_axis)
                    if phase == "descend":
                        dist = float(np.linalg.norm(data.site_xpos[site_id] - data.qpos[obj_qadr:obj_qadr+3]))
                        distance_during_descend.append(dist)
    ...
    if distance_during_descend:
        print(f"descend: min dist = {min(distance_during_descend):.4f}   "
              f"max dist = {max(distance_during_descend):.4f}   "
